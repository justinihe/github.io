...
(function () {
    ...
    function DisplayLoginPage() {
        ...
        (document.querySelector(".form-signin") as HTMLButtonElement).addEventListener("submit", async function (event) {
            event.preventDefault(); // Prevent form from refreshing the page

            const email = (document.getElementById("floatingInput") as HTMLInputElement).value.trim();
            const password = (document.getElementById("floatingPassword") as HTMLInputElement).value.trim();
    ...
    async function DisplayNewsPage() {
        ...
        const newsDiv = document.getElementById("newsDiv") as HTMLDivElement;
        try {
            ...
            type Data = { title: string, description: string, image_url: string };
    ...
    async function DisplayGalleryPage() {
        let displayedHtml = "";
        const galleryDiv = document.getElementById("galleryDiv") as HTMLDivElement;
        ...
        try {
            ...
            type Data = { title: string, description: string, image: string };
                ...
                modal.addEventListener('show.bs.modal', function (event) {
                    ...
                    if (img) {
                        ...
                        if (imageSrc && imageTitle && imageDescription) {
                            ...
                            const imageModalLabel = document.getElementById('imageModalLabel') as HTMLElement;
                            const lightboxDescription = document.getElementById('lightboxDescription') as HTMLElement;
    ...
    function DisplayContactPage(){
        ...
        let submitMessaage = document.getElementById("submitMessaage") as HTMLElement;
        let contactInfo = document.getElementById("contactInfo") as HTMLElement;
        let fullNameContact = document.getElementById("fullNameContact") as HTMLInputElement;
        let emailContact = document.getElementById("emailContact") as HTMLInputElement;
        ...
        submitMessaage.addEventListener("click", async e => {
            ...
            if(fullNameContact.value.trim().length <= 0 || emailContact.value.trim().length <= 0 || subject.value.trim().length <= 0 || message.value.trim().length <=0) {
                ...
                const modalConfirmation = new (window as any).bootstrap.Modal(document.getElementById("modalConfirmation"));
     ...
     */
    async function DisplayEventsPage(): Promise<void> {
        console.log("Calling DisplayEventsPage...");

        // Get references to DOM elements
        const searchNav = document.getElementById("searchNav") as HTMLInputElement | null;
        ...
        function renderEvents(filter: string = "all"): void {
            if (!calendar) return;
        ...
        searchNav.addEventListener("input", (e) => {
            renderEvents((e.target as HTMLInputElement).value);
        });

        categoryFilter.addEventListener("change", (e) => {
            renderEvents((e.target as HTMLSelectElement).value);
        });
...
// Create the "Back to Top" button
    const backToTopBtn = document.createElement('button') as HTMLButtonElement;
...
// Scroll smoothly to the top when the button is clicked
    ...
    async function handlePageLogic(path:string) {
        ...
        navLinks.forEach(link => {
            ...
            const linkPath:string = (link.getAttribute("href") as string).replace("#", "");
